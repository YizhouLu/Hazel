...
static void DrawVec3Control(const std::string& label, glm::vec3& values, float resetValue = 0.0f, float columnWidth = 100.0f) {
	...
	float lineHeight2 = GImGui->Font->FontSize + GImGui->Style.FramePadding.y * 2.0f;
	ImVec2 buttonSize2 = { lineHeight2 + 50.0f, lineHeight2 };
	if (ImGui::Button(label.c_str(), buttonSize2)) {
		values.x = resetValue;
		values.y = resetValue;
		values.z = resetValue;
	}
...
void SceneHierarchyPanel::DrawComponents(Entity entity)
{
	...
	if (entity.HasComponent<TransformComponent>()) {
		...
		if (opened) {
			...
			auto rotationInDegrees = glm::degrees(transform.Rotation);
	...
	if (entity.HasComponent<CameraComponent>()) {
		...
		if (opened) {
			...
			if (camera.GetProjectionType() == SceneCamera::ProjectionType::Perspective) {
				float vertialFOV = glm::degrees(camera.GetPerspectiveVertialFOV());
				if (ImGui::DragFloat("Vertical FOV", &vertialFOV)) {
					camera.SetPerspectiveVertialFOV(glm::radians(vertialFOV));
				}
